...
describe('Category', () => {
  ...
  it('should add a new category', () => {
    ...
    expect(localStorage.getItem(LOCAL_STORAGE_KEY)).toBe(JSON.stringify([newCategory]))
  });
  
  it('should remove a category by ID', () => {
    const initialCategories = [{ id: '1', name: 'Test Category' }, { id: '2', name: 'Test Category' }];
    ...
    expect(localStorage.setItem).toHaveBeenCalled();
    expect(localStorage.getItem(LOCAL_STORAGE_KEY)).toBe(JSON.stringify([{ id: '2', name: 'Test Category' }]))
  ...
  it('should add a task to a category', () => {
    ...
    const task = { id: 'task1', name: 'Test Task', completed: false, remainingTime: [120]};
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(initialCategories))
    Category.addTasks('1', task);
  
    expect(localStorage.getItem).toHaveBeenCalled();
    expect(localStorage.setItem).toHaveBeenCalled();
    expect(localStorage.getItem('todo')).toBe(JSON.stringify([{ id: '1', name: 'Test Category', tasks: [task]}]))
  ...
  it('should calculate and update the completion ratio of a category', () => {
    const category = new Category('Test Category', [
      { id: 'task1', name: 'Test Task 1', completed: true, remainingTime: [120] },
      { id: 'task2', name: 'Test Task 2', completed: false, remainingTime: [60] },
    ]);
